...
public abstract class PetriNetTypeImpl extends EObjectImpl implements PetriNetType {
	...
	public RefPlace createRefPlace() {
		EClass typeClass = this.eClass();
		EPackage ePackage = typeClass.getEPackage();
		EClassifier objectClass = ePackage.getEClassifier("RefPlace");
		if (objectClass != null && objectClass instanceof EClass) {
			EObject object = ePackage.getEFactoryInstance().create((EClass) objectClass);
			if (object != null && object instanceof RefPlace) {
		    	return (RefPlace) object;
			}
		}
	...
	public RefTransition createRefTransition() {
		EClass typeClass = this.eClass();
		EPackage ePackage = typeClass.getEPackage();
		EClassifier objectClass = ePackage.getEClassifier("RefTransition");
		if (objectClass != null && objectClass instanceof EClass) {
			EObject object = ePackage.getEFactoryInstance().create((EClass) objectClass);
			if (object != null && object instanceof RefTransition) {
